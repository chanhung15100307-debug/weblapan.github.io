// read file data.js
import { type1, type2, type3, type4, type5, type6 } from "./data.js";

const row = document.querySelector(".row");

const data = [type1, type2, type3, type4, type5, type6];
...
function calSum() {
  ...
  currAddedItems.forEach((item) => {
    ...
    if (
      item.id !== "41" &&
      item.id !== "42" &&
      charges + item.point * item.quantity <= 1000
    ) {
      charges += item.point * item.quantity;
    } else if (charges + item.point * item.quantity > 1000) {
      charges = 1000;
    }
  });
...
function addRule(rule) {
  ...
  switch (rule) {
    case "military":
      ...
      const specialItem = currAddedItems.find((item) => item.id === "18");
    ...
    case "cooperation":
      ...
      break;
      case "Confess":
  const Conf = document.querySelector("#Confess-num").value;
  newItem = {
    id: "Confess",
    name: `(Giảm ${Conf} phút đầu thú)`,
    quantity: 1,
    isSelected: true,
    point: -parseInt(Conf),
    type: 0,
